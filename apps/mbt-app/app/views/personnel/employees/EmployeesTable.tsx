...
export default function EmployeesTable() {
  ...
  // Refresh data on mount and when component becomes visible
  // Filter out drivers from this table
  React.useEffect(() => {
  ...
  const columns = [
    ...
    columnHelper.accessor("role", {
      ...
      cell: (info) => {
        ...
        return (
          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${roleColors[info.getValue()] || 'bg-gray-100 text-gray-700'}`}>
            {info.getValue()}
          </span>
        );
      },
    ...
    <Card extra={"w-full px-6"}>
      <div className="flex items-center justify-between pt-[20px]">
        ...
        <button
          ...
          className="ml-4 flex items-center gap-2 rounded-xl bg-accent-500 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-600 active:bg-brand-700 dark:bg-brand-400 dark:hover:bg-brand-300"
        >
        ...
        <div className="mt-2 flex h-20 w-full items-center justify-between px-6">
          <div className="flex items-center gap-3">
            <p className="text-sm text-gray-700 dark:text-gray-300">
              Mostrando {table.getRowModel().rows.length} de {data.length} empleados
            </p>
          </div>
