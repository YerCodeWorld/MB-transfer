...
export default function Auth() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (!identifier.trim()) {
			setError("Por favor ingrese su correo o identificación");
			setIsLoading(false);
			return;
		}
		...
		try {
			const success = await authenticate(identifier.trim(), accessKey.trim());
	...
	return (
		<div className="relative flex min-h-screen w-full bg-white dark:bg-navy-900">
			...
			<div className="flex w-full items-center justify-center px-6 sm:px-10 lg:px-16">
				<div
				  className="w-full max-w-[520px] p-8 sm:p-10 bg-white/95 dark:bg-navy-700/90 border border-black/10 dark:border-white/10 shadow-[0_14px_38px_rgba(0,0,0,0.4)] backdrop-blur"
				>
				  ...
				  <form onSubmit={handleSubmit} className="space-y-6" aria-label="Formulario de acceso">
					...
					name="identifier"
					type="text"
					placeholder="correo@ejemplo.com o 001-1234567-8"
					value={identifier}
					...
					required
				      />
				      {error && (
					<div id="form-error" className="mt-2 flex items-center gap-2 text-red-600">
