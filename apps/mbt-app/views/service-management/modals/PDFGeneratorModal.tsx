...
interface PDFGeneratorModalProps {
  ...
  markServicePDFAsChecked: (serviceId: string, checked: boolean) => Promise<void>;
}
...
const PDFGeneratorModal: React.FC<PDFGeneratorModalProps> = ({
  ...
  const handleEditService = (service: ExtendedService) => {
    ...
    setEditFormData({
      ...
      checked: service.pdfData?.checked 
    });
  };
        ...
        <div className="overflow-y-auto p-6" style={{ maxHeight: 'calc(90vh - 120px)' }}>
          <div className="space-y-4">
            ...
            <div>
                ...
                {filteredServices.length === 0 ? (
                  ...
                  filteredServices.map((service) => {
                    ...
                    return (
                      <div
                        key={service.code}
                        className={`flex items-center justify-between p-4 dark:bg-navy-700 rounded-lg border border-gray-200 hover:shadow-md transition-shadow ${service.pdfData?.checked ? 'border-green-200 border-4' : 'bg-white'}`}
                      >
			  ...
			  <input 
				type="checkbox"
				checked={service.pdfData?.checked ?? false}
				onChange={ async (e) => {
					try {
						await markServicePDFAsChecked?.(service.id || "", e.target.checked);
					} catch (err) {
						alert(err);
					}
				}}
				className="w-4 h-4 border border-default-medium rounded-xs"
			  >
			  </input>
                          <button
        ...
        <div className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              Nombre del Cliente {editFormData.checked ? 'hello' : 'hi'}
            </label>
            <input
              type="text"
