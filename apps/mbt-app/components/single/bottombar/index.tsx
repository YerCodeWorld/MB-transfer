...
export default function BottomBar({ section, mini = false, className = "" }: BottomBarProps) {
  ...
  const getActionButtonClass = (variant?: string, active?: boolean, square?: boolean) => {
    const shapeClass = square ? "rounded-md" : "rounded-xl";
    const baseClass = `flex items-center justify-center gap-2 ${shapeClass} px-3 py-2.5 text-sm font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed`;

    if (active) {
      return `${baseClass} bg-accent-700 text-white hover:bg-accent-800 focus:ring-accent-500 shadow-lg`;
    }
        ...
        <div className="flex items-center gap-1 rounded-2xl border border-gray-200 bg-white/95 p-2 shadow-xl dark:border-gray-700 dark:bg-navy-800/95">
          {visibleActions.map((action) => (
            <button
              ...
              className={getActionButtonClass(action.variant, action.active, action.square)}
              title={action.label}
            >
